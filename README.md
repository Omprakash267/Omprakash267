<div align="center">

<img src="https://capsule-render.vercel.app/api?type=slant&color=0:0d0d0d,100:8B0000&height=220&section=header&text=OMPRAKASH%20SENTHILKUMAR&fontSize=42&fontColor=ffffff&animation=twinkling&fontAlignY=38&desc=🕷️%20Weaving%20code%20like%20webs%20🕸️&descAlignY=58&descSize=18" width="100%"/>

<img src="https://readme-typing-svg.demolab.com/?lines=%F0%9F%95%B7%EF%B8%8F+Hey%2C+I%27m+Omprakash;A+web-slinging+developer;Machine+Learning+%7C+Full-Stack+%7C+AI;Spinning+RAG+chatbots+%26+LSTM+models;Welcome+to+my+web...&font=Fira%20Code&size=24&pause=1200&color=E63946&center=true&vCenter=true&width=650&height=60&separator=;" alt="Typing SVG" />

</div>

<p align="center">
  <img src="https://user-images.githubusercontent.com/74038190/213866269-5d00981c-7c98-46d7-8a8e-16f462f15227.gif" width="100%">
</p>

<div align="center">

### 🕸️ *"With great commits comes great responsibility"* 🕸️

<img src="https://media.giphy.com/media/xUOxf48VHUXSlWJflO/giphy.gif" width="260" alt="spider crawling"/>

</div>

---

## 🕷️ About Me

```javascript
const omprakash = {
  location: "Coimbatore, India 🇮🇳",
  role: "Machine Learning & Full-Stack Developer",
  currentWeb: "AI-powered applications 🕸️",
  portfolio: "https://omprakash26.vercel.app",
  funFact: "I trap bugs before they trap my code 🐛➡️🕸️",
  motto: "Every codebase is a web waiting to be spun."
};
```

<img align="right" src="https://avatars.githubusercontent.com/u/219404970?v=4" width="180" style="border-radius:50%; border: 3px solid #8B0000;" alt="Omprakash267 avatar"/>

- 🔭 I'm currently spinning webs of code around **RAG chatbots**, **LSTM models**, and **full-stack apps**
- 🕸️ I like connecting nodes — literally (neural nets) and figuratively (people!)
- 🌱 Currently strengthening my silk... I mean, skills in **AI/ML & System Design**
- 👯 Looking to collaborate on **AI-powered** and **data-driven** web projects
- 🕷️ My GitHub contribution graph looks like a spider web when it's a good week
- 📫 Reach me: **[LinkedIn](https://linkedin.com/in/omprakash-senthilkumar-866a53289)** | **[Portfolio](https://omprakash26.vercel.app)**

<br clear="right"/>

---

<div align="center">

## 🕸️ My Web of Skills

<img src="https://media.giphy.com/media/VgCDAzcKvsR6OM0uWg/giphy.gif" width="120" align="left" alt="spider"/>

<img src="https://skillicons.dev/icons?i=python,js,ts,react,nextjs,html,css,tensorflow,git,vscode,figma,vercel&perline=6" />

<br clear="left"/>
</div>

---

## 🕸️ Featured Webs (Projects)

<table align="center">
<tr>
<td width="50%">

### 🕷️ [Intelligent Business RAG Chatbot](https://github.com/Omprakash267/Intelligent-Business-RAG-Chatbot)
Retrieves answers from local docs using **Chroma vector DB** + **Ollama LLM**, with hallucination guardrails, sentiment analysis & source citation.
`Python` `RAG` `LLM`

</td>
<td width="50%">

### 🌍 [Earthquake Magnitude Prediction (LSTM)](https://github.com/Omprakash267/EARTHQUAKE-MAGNITUDE-PREDICTION-USING-LSTM)
Deep learning web app predicting earthquake magnitude from geographical features using **LSTM** networks.
`Python` `Deep Learning`

</td>
</tr>
<tr>
<td width="50%">

### 🚀 [StartupKart](https://github.com/Omprakash267/StartupKart)
A startup-focused platform built with modern web tooling.
`TypeScript`

</td>
<td width="50%">

### 🌐 [IP Location Finder](https://github.com/Omprakash267/IP-Location-Finder)
Traces IP addresses to real-world locations — like tracking a fly caught in the web.
`CSS` `JavaScript`

</td>
</tr>
<tr>
<td width="50%">

### 📊 [World Population Visualization](https://github.com/Omprakash267/World-Population-Visualization)
Interactive visualization spinning global population data into insight.
`Python`

</td>
<td width="50%">

### 🩺 [Diabetes Type Identifier](https://github.com/Omprakash267/Diabetes-Type-Identifier)
A web tool that helps identify diabetes type from health parameters.
`HTML`

</td>
</tr>
</table>

---

<div align="center">

## 🕷️ Web Stats

<img src="https://github-readme-stats.vercel.app/api?username=Omprakash267&show_icons=true&theme=dark&hide_border=true&bg_color=0d0d0d&title_color=E63946&icon_color=8B0000&text_color=ffffff&count_private=true" width="48%" alt="GitHub stats"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Omprakash267&theme=dark&hide_border=true&background=0d0d0d&ring=8B0000&fire=E63946&currStreakLabel=E63946" width="48%" alt="GitHub streak"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Omprakash267&layout=compact&theme=dark&hide_border=true&bg_color=0d0d0d&title_color=E63946&text_color=ffffff" width="45%" alt="Top languages"/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Omprakash267&theme=react-dark&bg_color=0d0d0d&color=E63946&line=8B0000&point=ffffff&hide_border=true" width="90%" alt="Activity graph"/>

</div>

---

<div align="center">

## 🕸️ Catch Me On the Web

<a href="https://linkedin.com/in/omprakash-senthilkumar-866a53289" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-8B0000?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="https://omprakash26.vercel.app" target="_blank">
  <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio"/>
</a>
<a href="https://github.com/Omprakash267" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-0d0d0d?style=for-the-badge&logo=github&logoColor=E63946" alt="GitHub"/>
</a>

<br/><br/>

<img src="https://media.giphy.com/media/3o7abGQa0aRJUurpwI/giphy.gif" width="200" alt="spider web spin"/>

### 🕷️ Thanks for visiting my web — don't get too tangled up! 🕸️

<img src="https://komarev.com/ghpvc/?username=Omprakash267&label=Flies%20caught%20in%20the%20web&color=8B0000&style=flat" alt="profile views"/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:8B0000,100:0d0d0d&height=120&section=footer"/>

</div>
